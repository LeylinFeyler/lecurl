...
static struct option long_options[] = {{"connect-timeout", required_argument, 0, 1000},
...
int cli_parse(int argc, char **argv, CliOptions *opts) {
    ...
    while ((opt = getopt_long(argc, argv, "o:IiH:X:d:u:Lh", long_options, NULL)) != -1) {
        switch (opt) {
        case 'o':
